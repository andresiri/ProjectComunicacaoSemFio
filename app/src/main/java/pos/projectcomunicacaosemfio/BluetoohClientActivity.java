 ...
 */
public class BluetoohClientActivity extends Activity {
    ...
    private TextView tv_fluxoar;
    private TextView tv_posicaoborb;
                        ...
                        public void run() {
                            while(true) {
                                try {
                                    ...
                                    Thread.sleep(100);
                                    out.write("01 11\r".getBytes()); // posicao borboleta
                                    Thread.sleep(100);
                                    out.write("01 10\r".getBytes()); // fluxo ar
                                    Thread.sleep(100);
                                } catch (IOException e) {
                                    e.printStackTrace();
            ...
            public void run() {
                try {
                    ...
                    if (deucerto.substring(0,5).equals("41 0C")) { // Rotação do motor
                        ...
                        tv_rotacao.setText("Rotação: "+(convertHexa(numero)*256 + convertHexa(numero2))/4 + "rpm");
                    }
                    if (deucerto.substring(0,5).equals("41 0D")) { // Velocidade
                        String numero = deucerto.substring(6,8);
                        tv_velocidade = (TextView) findViewById(R.id.tv_velocidade);
                        tv_velocidade.setText("Velocidade: "+convertHexa(numero) + " km/h");
                    }
                    if (deucerto.substring(0,5).equals("41 05")) { // temperatura
                        String numero = deucerto.substring(6,8);
                        tv_temperatura = (TextView) findViewById(R.id.tv_temperatura);
                        tv_temperatura.setText("Temperatura: "+(convertHexa(numero)-40)+" C");
                    }
                    if (deucerto.substring(0,5).equals("41 11")) { // posicao borboleta
                        String numero = deucerto.substring(6,8);
                        tv_posicaoborb = (TextView) findViewById(R.id.tv_posicaoborb);
                        tv_posicaoborb.setText("Posição Borboleta: "+(convertHexa(numero)*100)/255+" %");
                    }
                    if (deucerto.substring(0,5).equals("41 10")) { // fluxo de ar
                        String numero = deucerto.substring(6,8);
                        String numero2 = deucerto.substring(9,11);
                        tv_fluxoar = (TextView) findViewById(R.id.tv_fluxoar);
                        tv_fluxoar.setText("Fluxo de ar: "+((convertHexa(numero)*256 + convertHexa(numero2))/100)+" g/s");
                    }
